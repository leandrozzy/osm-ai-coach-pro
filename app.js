...
const APP_VERSION='5.0.0-cloud-strategy';
...
async function handleFiles(files){
 ...
 try{
   ...
   if(!['calendar','result'].includes(analysisMode)){applyVisionResult(result);if(analysisMode==='market'){const ss=state.slots[selectedSlot-1];ss.lastRosterSnapshotAt=nowIso();ss.lastMarketSnapshotAt=nowIso();}}
   ...
   toast(analysisMode==='tactic'?'Partida analisada e tática gerada':analysisMode==='calendar'?'Calendário atualizado':analysisMode==='result'?'Resultado registrado e aprendizado atualizado':'Mercado atualizado');
 }catch(e){
   console.error(e);
...
async function analyzeCalendarOcr(ocr,evidence){
 ...
 const parts=[{text:`Você é um extrator de CALENDÁRIO do OSM 26. Analise o OCR e as imagens do calendário do Slot ${selectedSlot}.
Horário padrão cadastrado: ${slot.defaultMatchTime||'não informado'}.
REGRAS: placar visível = já jogado (played=true); ícone de casa à esquerda = campeonato em Casa; partida de campeonato sem casa = Fora; ícone de taça/troféu = competitionType=\"cup\" e conditional=true enquanto depender de classificação. Extraia rodada, adversário, data e horário visíveis. Se faltar horário, use o horário padrão e marque timeSource=\"default\". Não invente.
Retorne SOMENTE JSON: {\"matches\":[{\"round\":null,\"opponent\":null,\"competitionType\":\"league|cup|null\",\"venue\":\"Casa|Fora|null\",\"dateText\":null,\"timeText\":null,\"dateTime\":null,\"timeSource\":\"screen|default|null\",\"result\":null,\"played\":false,\"conditional\":false}]}
OCR:\n${ocr.joined}`}];
 for(const f of evidence)parts.push({inlineData:{mimeType:f.mimeType,data:f.base64}});
 return geminiJson(parts,{temperature:.01,maxOutputTokens:6500});
}
function normalizeCalendarMatch(x){return {round:x.round??null,opponent:x.opponent??null,competitionType:x.competitionType||null,venue:x.venue||null,dateText:x.dateText||null,timeText:x.timeText||null,dateTime:x.dateTime||null,timeSource:x.timeSource||null,result:x.result||null,played:!!x.played,conditional:!!x.conditional,skipped:false}}
function cupLossFromResult(r){if(!r)return false;const m=String(r).match(/(\d+)\s*[-x:]\s*(\d+)/i);return !!m&&Number(m[1])<Number(m[2])}
function pruneConditionalCupMatches(s){let eliminated=false;for(const x of s.schedule||[]){if(x.competitionType!=='cup')continue;if(x.played&&cupLossFromResult(x.result)){eliminated=true;continue}if(eliminated&&!x.played){x.skipped=true;x.conditional=true}}}
...
4) Analista — resumo
5) Analista — marcação e impedimento
6) Analista — formação
...
function strong433Modal(n){
 const s=state.slots[n-1],o=s.opponent||{},m=s.match||{};openModal(`<h2>🔥 4-3-3 Forte · Slot ${n}</h2><p class="muted small">Disponível quando sua força é pelo menos 15 pontos maior. Informe o básico atual; a IA usa também o histórico do slot.</p><div class="form-card"><div class="data-grid"><label>Minha força<input id="s43my" type="number" value="${esc(s.myTeam.overall??'')}"></label><label>Força rival<input id="s43opp" type="number" value="${esc(o.overall??'')}"></label></div><label>Local<select id="s43venue"><option${m.venue==='Casa'?' selected':''}>Casa</option><option${m.venue==='Fora'?' selected':''}>Fora</option></select></label><label>Árbitro<input id="s43ref" value="${esc(m.refereeColor||m.refereeName||'')}"></label><label>Formação rival<input id="s43form" value="${esc(o.formation||'')}"></label><label>Plano rival<input id="s43style" value="${esc(o.style||'')}"></label><label>Marcação rival<input id="s43mark" value="${esc(o.marking||'')}"></label><label>Impedimento rival<select id="s43off"><option>NI</option><option${o.offside===false?' selected':''}>Não</option><option${o.offside===true?' selected':''}>Sim</option></select></label><label>Rival humano?<select id="s43human"><option>NI</option><option${o.human===true?' selected':''}>Sim</option><option${o.human===false?' selected':''}>Não</option></select></label><button class="btn" onclick="generateStrong433(${n})">Gerar 4-3-3 forte</button></div>`)}
async function generateStrong433(n){
 ...
 const basic={myOverall:my,oppOverall:opp,venue:document.getElementById('s43venue').value,referee:document.getElementById('s43ref').value||null,oppFormation:document.getElementById('s43form').value||null,oppStyle:document.getElementById('s43style').value||null,oppMarking:document.getElementById('s43mark').value||null,oppOffside:document.getElementById('s43off').value,oppHuman:document.getElementById('s43human').value,history:buildLearningSummary(s)};
...
Object.assign(window,{showView,setSelectedSlot,prepareReanalysis,configureSlot,saveSlotConfigV5,openCalendarForSlot,finishCompetition,confirmFinish,slotDetailModal,generateTacticForSlot,tacticModal,manualTacticModal,saveManualTactic,strong433Modal,generateStrong433,resultModal,saveResultV5,scheduleModal,saveSchedule,downloadIcs,openMarketSnapshot,aiStrategyReview,strategyModal,renderInfo,updateEventIntel,saveApiKey,clearApiKey,closeModal});
